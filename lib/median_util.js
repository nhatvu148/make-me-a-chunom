...
const findStrokeMedian = (stroke) => {
  ...
  // If Voronoi fails, return a simple fallback median from polygon endpoints
  if (!diagram) {
    console.warn('Voronoi computation failed, using fallback median');
    if (polygon && polygon.length >= 2) {
      // Return start and end points as a simple median
